...
class ViewRoyalController extends Controller
{
    public function royal_unit(Request $request)
    {
        $unit = Unit::all();
 ...
 public function royal_master_size(Request $request)
 {
    $product = $request->input('pname');
    $uid = $request->input('uid');
    $mid = $request->input('mid');

     $royalMattress = RoyalMattress::select('*')
    ->whereIn('id', function ($query) use ($product, $mid) {
        $query->select(DB::raw('MAX(id)'))
            ->from('royal_mattresses')
    ...
    if($uid == 1){
             foreach($royalMattress as $royalMattress){
            $sizeMM = SizeMM::where('id', $royalMattress->size_mm)->get();
        ...
        foreach($royalMattress as $royalMattress){
        $sizeInch = SizeInch::where('id', $royalMattress->size_inch)->get();
   ...
   public function royal_size_id(Request $request)
{
    $thickness_id = RoyalMattress::where(function ($query) use ($request) {
...
// dd($thickness_id);
    ...
    public function royal_thick_id(Request $request)
    {
        $thickness_id = RoyalMattress::where('thickness_id',$request->thick_id)->first();
    ...
    public function royal_price(Request $request)
    {
        // dd($request);
        // die();


        $price = RoyalMattress::where([
