            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        <h6>Add Size in MM For Pillows</h6>
                    </div>
                    <div class="card-body px-0 pt-0 py-2">
                        <form class="px-5" action="insert_pillow_size_mm" method="POST">
                            @csrf
                           
                            <div class="d-flex">
                                <div class="mb-3 mx-3 w-50">
                                    <label for="size_length_mm" class="form-label">Length MM</label>
                                    <input type="text" class="form-control" name="size_length_mm" id="size_length_mm" required>
                                </div>

                                <div class="mb-3 mx-3 w-50">
                                    <label for="size_breadth_mm" class="form-label">Breath in MM</label>
                                    <input type="text" class="form-control" name="size_breadth_mm" id="size_breadth_mm" required>
                                </div>
                            </div>
                                    ...
                                    @foreach($size_pillow as $item)
                                       <?php  
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$item->size_length_mm}}</td>
                                            <td>{{$item->size_breadth_mm}}</td>
                                            
                                            <td>{{ $unit->unit }}</td>
                                            
                                            <td class="align-middle">
                                                <a href="{{ url('size_pillows_mm/edit/' . $item->id) }}"
                                                    class="badge btn btn-success">Edit</a>
                                                <a href="{{ url('size_pillows_mm/delete/' . $item->id) }}"
