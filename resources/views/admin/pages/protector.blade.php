            ...
            <div class="col-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body px-0 pt-0 py-2">
                        <form class="px-5" action="/insert_protector" method="POST"  enctype="multipart/form-data">
                            @csrf
                            <div class="d-flex">
                            ...
                            <table class="table align-items-center mb-0" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase">Image 1</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach($protector as $item)
                                        <?php
                                        $size = Illuminate\Support\Facades\DB::table('size_protectors')
                                            ->where('id', $item->size)
                                            ->first();
                                        ...
                                        <tr>
                                           ...
                                           <td>{{$item->product}}</td>
                                           <td>{{$item->category}}</td>
                                           <td>{{ $size->length }} x {{ $size->width }} {{ $u_inch->unit }}</td>
                                           <td>{{$item->price}}</td>
                                           <td><img src="{{ asset('storage/images/' . $item->image1) }}"
                                           ...
                                           <td><img src="{{ asset('storage/images/' . $item->image2) }}"
                                            ...
                                            <td class="align-middle">
                                                <a href="{{ url('protector/edit/' . $item->id) }}"
                                                    class="badge btn btn-success">Edit</a>
                                                <a href="{{ url('protector/delete/' . $item->id) }}"
                                                    onclick="return confirm('Are you sure you want to delete this protector?')">
