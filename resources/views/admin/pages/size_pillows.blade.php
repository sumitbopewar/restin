            ...
            <div class="col-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body px-0 pt-0 py-2">
                        <form class="px-5" action="insert_pillow_size" method="POST">
                            @csrf
                           
                            <div class="d-flex">
                                <div class="mb-3 mx-3 w-50">
                                    <label for="size_length" class="form-label">Length Inch</label>
                                    <input type="text" class="form-control" name="size_length" id="size_length" required>
                                </div>

                                <div class="mb-3 mx-3 w-50">
                                    <label for="size_breadth" class="form-label">Breath in Inch</label>
                                    <input type="text" class="form-control" name="size_breadth" id="size_breadth" required>
                                </div>
                            </div>
                                    ...
                                    @foreach($size_pillow as $item)
                                       <?php  
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$item->size_length}}</td>
                                            <td>{{$item->size_breadth}}</td>
                                            
                                            <td>{{ $unit->unit }}</td>
                                            
                                            <td class="align-middle">
                                                <a href="{{ url('size_pillows/edit/' . $item->id) }}"
                                                    class="badge btn btn-success">Edit</a>
                                                <a href="{{ url('size_pillows/delete/' . $item->id) }}"
